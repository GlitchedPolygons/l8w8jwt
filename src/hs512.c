...
int l8w8jwt_encode_hs512(struct l8w8jwt_encoding_params* encoding_params)
{
    ...
    r = encode(&stringbuilder, L8W8JWT_ALG_HS512, encoding_params);
    if (r != L8W8JWT_SUCCESS)
    {
    ...
    r = mbedtls_md_hmac(&mbedtls_sha512_info, encoding_params->secret_key, encoding_params->secret_key_length, (const unsigned char*)stringbuilder.array, stringbuilder.length, (unsigned char*)signature_bytes);
    if (r != 0)
    {
        chillbuff_free(&stringbuilder);
        return L8W8JWT_HS512_SIGNATURE_FAILURE;
    }
